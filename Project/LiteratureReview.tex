...
Modern CTI which often relies on a "fixed-point" monitoring of curated data feeds from sources like security organizations and white papers \cite{zhang1}.
...
A desktop computer has a more powerful CPU, and is chained to a wall. But a phone has much more stringent power, heat, and battery constraints when the topic of machine learning is concerned\cite{sonowal1}.
...
That reduction in dimensionality was key in making the model viable for mobile devices as well.
...
AI tools like that can provide real-time feedback, and can even adapt to each user's understanding level, allowing the aforementioned accountability to come more naturally \cite{dash1}.

...
\node[block, below left=of THRESH] (CLASS) {Data Wrangling \&\\Classification of Cisco's unknown "Other"-type phishes};
...
%Discuss how your findings relate to or differ from existing studies.
TODO!
...
As this work moves into the future, a huge aspect can be incorporating additional feature construction: TLD categorization, whether the URL contains brand keywords, character n-gram frequencies, clustering results, or Levenshtein distance to known legitimate domains. We could also compare the SVM against other classifiers (Random Forest, Gradient Boosting, MLP) to benchmark and likely even improve performance. After that, ddditional datasets could be used to augment and mitigate the biases of PhishTank and Tranco, as more data could allow more dimensions to arise and outliers to be found. And after extracting the the linear SVM coefficients, we could use them to inform feature selection for the overall phishing detection pipeline, which could improve outcomes aswell. All in all, the research was a success, and showed that the often neglected base datum of a phishing domain is rich with semantic and ML potential for attack prevention.

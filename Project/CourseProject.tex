...
% \IEEEoverridecommandlockouts % The preceding line is only needed to identify funding in the first footnote. That's unneeded, so we comment it out.
\usepackage{cite}
...
This paper presents a machine learning pipeline designed to classify multiple malicious phishing and impersonation criteria, thus allowing a large proportion of malicious URLs to be proactively remediated.
A robust lexical feature set was extracted from phishing URLs, including path depth and digit ratios.
From there, to automatically establish a basis for impersonation, we utilized cosine similarity scores between Phishtank's known phishing URLs and the Tranco legitimate domain dataset, applying a 75th-percentile threshold on semantic similarity to create a binary target variable.
This target represents mined and cleaned semantic metadata. From there, two AI methodologies were applied to this: the first was a K-Means clustering model for overall extraction of malicious methodologies. 
After that, we honed in on the attack methodology of "impersonation" with a Support Vector Machine. It's RBF kernel was trained and optimized via randomized grid search to reach the "proactive" goal of our paper, with it being able to work on novel data.
This allowed ML classification to pick up after the semantic extraction and automatically classify domains of note. 
Performance for all models was rigorously evaluated to ensure efficacy, and their results were correlated in a human-readable triage tool.
Those deliverables showed our methodology successfully demonstrating that structural URL characteristics can effectively capture impersonation intent. With tools like this, semantic classification tasks like impersonation detection are more available for future research, showcasing that the industry has many untapped avenues for domain metadata.
\end{abstract}

\section{Introduction} %@j-recasens @ns15468-gasou research question has to be added explicitly below for full points
...
As such, the realm of machine learning and predictive analytics is of known importance in Cybersecurity research \cite{brezeanu_phish_2025}.
...
But with Ransomware and Malware becoming billion dollar multi-national black markets, we need to be able to apply risk trends to novel data \cite{desolda_apollo_2025}.
...
And if they are used successfully, they become indicators of compromise (IoC) \cite{saidani1}.
...
Over the course of this paper, our research will aim to improve the proactive identification of cybersecurity threats by mining semantic relevance from raw domain data to enhance prior results. Our study’s layout will begin with a comprehensive literature review to show the lack of focus on domain level data in current research. From there, we will look at data stores of active domains and a repository of malicious URLs, and work to bridge the gap between raw data and actionable intelligence. We will utilize text embeddings to generate a vector space understanding of semantic indicators, condensed through cosine similarity metrics. That data then fuels a two-phased ML approach where a Support Vector Machine (SVM) is trained to predict impersonation risks, acting as a "defense in depth" mechanism for indicator analysis, while also applying unsupervised K-Means clustering to identify four distinct phishing archetypes, such as platform abuse and link shortener redirection, providing a clearer look into the more general categorization of threats. Together, we can show in our results highlight how the correlation between the two shows the untapped potential a raw phishing domain has for detection.
...
These 2 combined allow us to glean basic lexical and numerical insights on the data typically used by spam detection. Figure \ref{fig:datshape} shows how stratified the 2 datasets can be with their features:
...
\end{figure}
\clearpage
As expected from that, the data had to be cleaned and normalized for us to do meaningful analysis.
...
And for the IoCs specifically, they have an issue of a large chunk of them being hosted by a legitimate (or at least largely used) hosting platform. As shown in Figure \ref{fig:dedup}, many attackers use the perceived legitimacy of larger sites to hide in the crowd, and to create malicious payloads that HTTP based scanners may not be able to identify quickly. This means when we train our ML later, we will need to filter out these hosting domains.
...
Once embeddings are made and compared, they will have to be stored and aggregated, as the raw results of our team's cosine similarity data came out to 23GB of data, with over 2.7 billion vector comparisons. That is too much to do in RAM alone on Colab, so we process the data line by line and store to disk. From there, we did exploratory correlation, classification, and novel data testing, which let us decide on our final methodology for this research. In the next section, we will glean insights from this on how the data's semantic layer adds on top of the capabilities exposed in the literature review. Table \ref{fig:embed} shows the basic shape of the embedded AI data as a preview of this analysis, showing us how the semantic correlation obviously exists, and giving us the push to look for lexical correlation.
...
With modern tools like vector databases, we could see already effective tools like iMCircle and Apollo gain a new semantic level of data for automatic decision making with textual embeddings \cite{desolda_apollo_2025, zhang1}. But for our process, we will use the closeness/similarity of the vectors as a filter, a sort of "sift" that allows us to train an SVM on the URL and it's features when a semantic hit occurs on the URL. The explained steps in preprocessing each feed into each other, and through our diligent process of fulfilling our desired framework, our team has the resources to provide answers and context to our research question about the efficacy of domain semantics. Figure \ref{fig:framework} on the next page shows our goals mentioned so far in a formalized Framework Diagram. From there, we begin with the first phase after preprocessing: feature extraction. 
\clearpage
...
\node[block, below left=of THRESH] (CLASS) {K-Means \& GBT\\of ``Other'' Phishes\\\& Attack Pattern Analysis};
...
As mentioned in the previous section, Scikit-Learn was chosen as our coding library, with Python 3.14 over Google Colab being our processing. All of these are industry standard choices for base level research, and worked together well for our purposes \cite{danchev_2022, inria}. For our library imports that implement the above goals, the appendix has heavily annotated code that shows our usage, and our reasoning for each. These imports are the glue and facilitators of our above model considerations in the code, and are available in an easy to understand visual Notebook form on our GitHub repo (linked in the appendix).
...
As a benchmark comparison, a Gradient Boosting classifier was trained on the very same lexical features and impersonation binary label as the SVM, achieving an AUC of 0.898 over the SVM's 0.809. The precision for both remained comparable at 0.86, though the recall on impersonating classes dropped to 0.53 without the balanced class weighting that was applied to the SVM.

\subsection{Clustering Results}
K-Means clustering was performed on the ``Other'' URLs as found in the PhishTank dataset, which account for 90\% of the \textasciitilde56,000 data points (Figure \ref{fig:clust}). Four groups or clusters were chosen based on the highest number of clusters that still introduced minimal noise. As seen in the elbow curve, we might think three clusters to be equally or more valid, especially in light of two similar clusters (simple and complex domain abuse), but their feature profiles are different enough to warrant this separation at four clusters. A Gradient Boosting classifier was then implemented, training on the cluster assignments as labels, achieving 99.45\% accuracy on a held-out test set. This high accuracy is expected given that GBT is reproducing the same cluster boundaries with the same feature space. The practical value in this endeavor is that new URLs can now be assigned to a cluster via supervised classification without rerunning K-Means.
...
\tableofcontents
%\newpage % New page clears the column instead of whole page
\listoftables
\listoffigures
